...
- `scripts/verify_deps.sh`, a dependency check that runs inside the container: it compares the installed packages against the local checkout's `uv.lock` and, with `--install`, installs the lock's versions for whatever drifted. `torch`, `torchvision`, `triton`, the `nvidia-*` runtimes and `rsl-rl-lib` are excluded, the same family `scripts/install_simulation.sh` keeps out of the lock export, so the CUDA stack is never reinstalled from the PyPI pins. Exits 3 on drift, 1 on error.
- `aegis_gym_toolbx` runs that check on every create, recreate **and join**, and offers to install what drifted. Until now the only Python action on any of those paths was `uv pip install --no-deps --editable`, so a dependency bump in `uv.lock` could not reach an existing container: the deps were frozen into `ceai/aegis_gym:<ver>` at image build time, from the `uv.lock` of the branch cloned then rather than from your checkout, and `require_base_image` deliberately refuses to rebuild that image. `--no-verify` skips the check.
